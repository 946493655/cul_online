...
<style>
    ...
    #menu .menu { margin:0;padding:5px 0;padding-left:5px;border-bottom:1px dashed #212121; }
    ...
    #menu input,#menu select { margin:2px 0;padding:2px 4px;width:20px;border:0;color:orangered;font-size:16px;background:#333; }
    #menu select { width:65px; }
    ...
    #submit { padding:5px 10px;width:170px;background:#1a1a1a;position:fixed;top:420px;left:802px;z-index:10; }
...
{{--动画窗口--}}
<div id="probg">{{--模板大背景--}}
    @if(isset($proAttrArr['isbg'])&&$tproAttrArr['isbg']==2)
...
<div id="menu">
    ...
    @if($layers)
        ...
        <div class="menu"><a href="javascript:;" title="点击切换" onclick="getMenu(3);">内容：</a>
            ...
            <span id="istext"><br>
                ...
                <form action="{{DOMAIN}}u/pro/{{$product['id']}}/layer/toimg/{{$layers['id']}}" method="POST" data-am-validator enctype="multipart/form-data" id="isimg" style="display:none;">
    ...
    filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod=scale);"></div>
        ...
        @if($hasRedis)
            <div id="submit">
                <a href="javascript:;" title="点击取消修改" class="submit" onclick="cancel()">取消修改</a><a href="javascript:;" title="点击确定修改" class="submit" onclick="save()">保存修改</a>
            </div>
        @endif
    @else
...
<script>
    ...
    function setLayer(){
        ...
        $.ajax({
            type: 'POST',
            url: '/u/pro/'+pro_id+'/layer/tolayer',
            data: data,
            dataType: 'json',
    ...
    //取消、删除几个缓存
    function cancel(){
        window.location.href = '{{DOMAIN}}u/pro/'+pro_id+'/layer/cancel/'+layerid;
    }
    //缓存入库
    function save(){
        window.location.href = '{{DOMAIN}}u/pro/'+pro_id+'/layer/save/'+layerid;
    }
</script>