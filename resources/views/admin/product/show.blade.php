...
@section('content')
        ...
        <table class="list">
            ...
            <tr><td colspan="5" style="border:0;">
                ...
                <div id="editpro">
                    ...
                    <a href="{{DOMAIN}}admin/pro/{{$data['id']}}/layer">细节修改</a>
                    <span id="isshow">
                        @if($data['isshow']==2)
                    ...
                    <a href="javascript:void(0);" title="彻底删除模板" onclick="getDel()">删除产品</a>
                    @endif
                </div>
            </td></tr>
        </table>
    ...
    <div class="editproduct" id="editproduct1">
        <div class="mask"></div>
        <form action="{{DOMAIN}}admin/product/{{$data['id']}}" method="POST" data-am-validator>
    ...
    <div class="editproduct" id="editthumb">
        <div class="mask"></div>
        <form action="{{DOMAIN}}admin/product/thumb/{{$data['id']}}" method="POST"
    ...
    <div class="editproduct" id="editlink">
        <div class="mask"></div>
        <form action="{{DOMAIN}}admin/product/link/{{$data['id']}}" method="POST"
              data-am-validator enctype="multipart/form-data">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <input type="hidden" name="_method" value="POST">
            <input type="hidden" name="uid" value="{{$data['uid']}}">
    ...
    <script>
        function getReturnPre(){ window.location.href = '{{DOMAIN}}admin/product'; }
        function getPlay(){
        ...
        function setShow(isshow){
            ...
            $.ajax({
                type: 'POST',
                url: '{{DOMAIN}}admin/product/getshow/'+id,
