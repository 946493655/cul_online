...
@section('content')
        ...
        <table class="list">
            <tr><td colspan="5">
                    <a href="javascript:;" style="color:lightgrey;"
                       onclick="window.location.href='{{DOMAIN}}admin/pro/{{$product['id']}}/layer';">返回上一页</a>
                </td></tr>
            <tr>
                <td>模板名称：{{$product['name']}}</td>
                <td>动画数量：{{$product['layerNum']}}</td>
                <td>创建时间：{{$product['createTime']}}</td>
                ...
                <td colspan="5">
                    ...
                    @foreach($layers as $layer)
                        <div class="layer">
                            ...
                            <a href="{{DOMAIN}}admin/pro/{{$product['id']}}/{{$layer['id']}}/layer/setshow/{{$layer['isshow']==1?2:1}}"
            ...
            <tr><td colspan="5" style="border:0;">
                    <iframe id="preview" frameborder=0 scrolling=no src="{{DOMAIN}}admin/pro/preview/layers/{{$product['id']}}"></iframe>
                </td>
            </tr>
        </table>
    </div>
    ...
    {{--<script>--}}
        ...
        {{--function delSure(){--}}
            ...
            {{--$("#del_sure")[0].href = '{{DOMAIN}}admin/pro/{{$product['id']}}/'+layerid+'/layer/delete';--}}
        {{--}--}}
    {{--</script>--}}
@stop