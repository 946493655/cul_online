...
<div id="addkey">
    <form action="{{DOMAIN}}admin/t/{{$pro_id}}/{{$layerid}}/frame" method="POST" data-am-validator>
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" name="_method" value="POST">
        <input type="hidden" name="tempid" value="{{$pro_id}}">
...
<script>
    ...
    function selAttr(attr){
        ...
        $.ajax({
            type: 'POST',
            url: '/admin/pro/'+pro_id+'/'+layerid+'/frame/toattr',
            data: data,
            dataType: 'json',
    ...
    function setFrame(frameid){
        ...
        $.ajax({
            type: 'POST',
            url: '/admin/pro/'+pro_id+'/'+layerid+'/frame/setval',
            data: data,
            dataType: 'json',
            success: function(data) {
                if (data.code!=0) {
    ...
    function cancel(){
        window.location.href = '{{DOMAIN}}admin/pro/'+pro_id+'/'+layerid+'/frame/cancel';
    }
    function save(){
        window.location.href = '{{DOMAIN}}admin/pro/'+pro_id+'/'+layerid+'/frame/save';
    }
    function delFrame(frameid){
        var attr = $("input[name='attr']").val();
        $.ajax({
            type: 'POST',
            url: '/admin/pro/'+pro_id+'/'+layerid+'/frame/delete',
